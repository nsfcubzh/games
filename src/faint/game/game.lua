    ...
    game.event = LocalEvent:Listen(LocalEvent.Name.DidReceiveEvent, function(e)
        Network:ParseEvent(e, {
            loadWorld = function(event)
                Debug.log(f"game() - received world with {event.data.map.blocks.Length} blocks, {event.data.map.objects.Length} objects and {event.data.map.coverings.Length} coverings.")
